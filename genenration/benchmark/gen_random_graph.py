...
def create_nocycle_graph(min_nodes, max_nodes, max_edges, minimum, maximum, weight=False, directed=False) :
    """
    仅生成无环图（DAG 或无向森林），其余接口与 create_random_graph 完全一致。
    """
    # 1. 合法性检查
    ...
    lower_limit = max(0, int(minimum * max_possible_edges))
    upper_limit = min(max_edges, int(maximum * max_possible_edges))
    num_edges = randint(lower_limit, max(upper_limit, 1))

    # 5. 生成无环边
    if directed:
        # 有向无环：随机拓扑排序后只允许“小→大”边
        perm = sample(nodes, num_nodes)           # 随机拓扑序
        allowed_edges = [(perm[i], perm[j])
                         for i in range(num_nodes)
                         for j in range(i + 1, num_nodes)]
    else:
        # 无向无环：随机生成森林（等价于随机选边，只要不成环即可）
        # 简单做法：随机选 num_edges 条边，再过滤出 acyclic 部分
        allowed_edges = list(combinations(nodes, 2))

    # 随机选取边，直到满足 num_edges 且不出现环
    selected = []
    for u, v in sample(allowed_edges, len(allowed_edges)):
        if len(selected) >= num_edges:
            break
        G.add_edge(u, v)
        if directed and not nx.is_directed_acyclic_graph(G):
            G.remove_edge(u, v)      # 有向图：破坏环
        elif not directed and not nx.is_forest(G):
            G.remove_edge(u, v)      # 无向图：破坏环
        else:
            selected.append((u, v))
