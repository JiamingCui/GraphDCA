...
def translate(G, args, number,ques,node,exit,edge,i):
    Q = ''
    ...
    Q = Q + "If there is a cycle, the final answer should only include 'Yes'. "
    Q = Q + "If there is no cycle, you should list all connected exit node pairs. And answer it started with 'Final Answer:'"
    return Q
...
def predict(Q, args):
    ...
    for text in input:
        ...
        try:
            ...
            max_tokens=args.token-len(text),
            stream=False,
            timeout=100  
            )
...
def extract_cluster_info(text,j,args):  
    ...
    newpath = 'multi_gpt/cycle/i'+args.source_path+'_'+args.number+'/'
...
def main():
    ...
    for i in tqdm(range((g_num + batch_num - 1) // batch_num)):
        ...
        for j in range(start_idx, start_idx+1):
            ...
            ques = None
            G=None
            Q = translate(G, args, j,ques,node,exit,edge,args.number)
            Q_list.append(Q)
