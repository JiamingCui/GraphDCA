...
def translate(G, q, args, number,edge):
    ...
    Q = Q +"The connectivity of node "+str(q[0])+" to the exit nodes in its cluster0 are as follows:"+'\n'

    file_path = "multi_gpt/con/i"+args.source_path+"_0/"+f"/graph{number}_0.txt"
    
    with open(file_path, "r", encoding="utf-8") as f:
        exemplar = f.read()
        Q = Q + exemplar + "\n"

    Q = Q +"The connectivity of node "+str(q[1])+" to the exit nodes in its cluster1 are as follows:"+'\n'

    file_path = "multi_gpt/con/i"+args.source_path+"_1/"+f"/graph{number}_1.txt"
    ...
    #Q = Q + "If there is no exit node connected to node "+str(q[0])+" or node "+str(q[1])+" , which means the annswer is 'No'."   # which means node "+str(q[0])+" and node "+str(q[1])+" are no connected."
