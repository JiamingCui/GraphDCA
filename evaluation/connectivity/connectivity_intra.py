...
def translate(G, q, args, number,ques,node,exit,edge,i):
    ...
    Q = Q + "Q: You should specify which exit "+ exit+" nodes in cluster 0 are connected to node "+str(q[0])+" and answer it started with 'Final Answer:'."
    Q = Q + "If node "+str(q[0])+" is belong to exit nodes, the final answer should include node "+str(q[0])+"."
    return Q
...
def extract_cluster_info(text,j,args):  
    ...
    newpath = 'multi_gpt/con/i'+'_'+args.number+'/'
